...
export async function authenticate(username: string, req_cookie: string) {
...
background_auth_router.use('*', async (req: Request, res: Response, next: NextFunction) => {
    ...
    const [resp, user] = await authenticate(req?.cookies?.username, req?.cookies?.cookie);
...
auth_router.post('/login', async (req: Request, res: Response) => {
    ...
    const user = (await collections.users?.findOne({ username: unsafe_payload?.username })) as unknown as User
